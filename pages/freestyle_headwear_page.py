...
class FreeStyleHeadwearPage(BasePage):
    def __init__(self, page: Page):
        super().__init__(page)

    def verify_freestyle_headwear_page_title(self):
        try:
            # 1 & 2. Validate that the text "FreeStyle Headwear" is displayed on the page as a title/heading.
            title_element = self.page.locator("text=/FreeStyle Headwear/i").first
            expect(title_element).to_be_visible(timeout=15000)
            # 3. Capture evidence
            self.report_step("Verified FreeStyle Headwear page title", "pass", snap=True)
    ...
    def verify_select_date_field_and_dropdown(self):
        ...
        try:
            ...
            # We skip the strict visibility check of calendar_popup container in case the class differs
            # and instead directly check if dates are visible
