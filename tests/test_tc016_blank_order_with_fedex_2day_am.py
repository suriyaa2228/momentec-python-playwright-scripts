...
# 15. Select FedEx 2 Day AM shipping method
...
def auth_context_tc016(request, env_config, browser_instance):
    """
    ...
    state_file = os.path.join(config_dir, "tc016_state.json")
...
@pytest.fixture(scope="class")
def auth_page_tc016(auth_context_tc016):
    page = auth_context_tc016.new_page()
    yield page
    page.close()

@pytest.mark.usefixtures("auth_page_tc016")
class TestTC016BlankOrderWithFedEx2DayAM:
    def test_place_order(self, auth_page_tc016, env_config):
        from python_playwright.utils.reporter import Reporter
        Reporter.start_test_case("TC016_BlankOrderWithFedEx2DayAM", "Verify able to place blank order with FedEx 2 Day AM Shipping method and product 225900", "Smoke", "SURIYAA")
        
        url = env_config["url"]
        
        auth_page_tc016.goto(url)
        home = HomePage(auth_page_tc016, url)
        home.handle_onetrust_cookie()
        # Clear cart to ensure no backordered items are present
        try:
            auth_page_tc016.goto(url + "AjaxOrderItemDisplayView?catalogId=10601&langId=-1&storeId=10251")
            from python_playwright.pages.cart_page import CartPage
        ...
        home_page.search_product("225900")
        
        # Assert navigation to PDP was successful by checking for a specific element or URL
        expect(auth_page_tc016).to_have_url(re.compile(".*(ProductDisplay|225900).*", re.IGNORECASE), timeout=15000)
            
        pdp = PDPPage(auth_page_tc016)
        pdp.verify_search_product("225900") \
            .select_color_black() \
            .verify_place_holder("225900") \
            .enter_quantity("225900") \
            .add_to_cart() \
        ...
        expect(auth_page_tc016).to_have_url(re.compile(".*(checkout|shipping|OrderShipping).*", re.IGNORECASE), timeout=15000)
            
        shipping = ShippingAndBillingPage(auth_page_tc016)
        shipping.verify_shipping_billing_page() \
            .click_shipping_methods_dd() \
            .select_fedex_2day_am() \
            .click_review_and_submit()
            
        review = ReviewSubmitPage(auth_page_tc016)
