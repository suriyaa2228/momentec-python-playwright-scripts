...
class TestTC008BlankOrderWithFedEx2Day:
    def test_place_order(self, auth_page_tc008, env_config):
        ...
        expect(auth_page_tc008).to_have_url(re.compile(".*(cart|CartView).*", re.IGNORECASE), timeout=15000)
        ...
        expect(auth_page_tc008).to_have_url(re.compile(".*(OrderOKView|ThankYou).*", re.IGNORECASE), timeout=30000)
